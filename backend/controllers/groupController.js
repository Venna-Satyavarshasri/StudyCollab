...
const getGroupDetails = asyncHandler(async (req, res) => {
  ...
  const isMember = populatedGroup.members.some(m => m._id.toString() === req.user._id.toString());
  if (!isMember) {
    res.status(403);
...
const deleteGroup = asyncHandler(async (req, res) => {
  ...
  // Notify all members in their personal rooms (they may not be in the group room)
  for (const memberId of group.members) {
    req.io.to(memberId.toString()).emit("group:deleted", groupId);
  }
...
const addMemberToGroup = asyncHandler(async (req, res) => {
  ...
  // Also notify the added user's personal room so their Dashboard updates instantly
  req.io.to(userToAdd._id.toString()).emit('group:joined', populatedGroup);
...
const removeMemberFromGroup = asyncHandler(async (req, res) => {
  ...
  const populatedAfterRemove = await Group.findById(groupId).populate('members', 'name avatar email');

  req.io.to(groupId).emit("group:member_removed", { memberId, group: populatedAfterRemove });

  // Also notify the removed user's personal room so their Dashboard updates instantly
  if (removedMember) {
    req.io.to(removedMember._id.toString()).emit('group:left', groupId);
  }
...
const joinGroupWithToken = asyncHandler(async (req, res) => {
  ...
  req.io.to(group._id.toString()).emit("group:member_added", {
    group: populatedGroup,
    member: { _id: joiningUser._id, name: joiningUser.name, avatar: joiningUser.avatar, email: joiningUser.email }
  });

  // Update joining user's Dashboard instantly
  req.io.to(userId.toString()).emit('group:joined', populatedGroup);
