...
const NotificationBell = () => {
  ...
  return (
    ...
    {/* Clear all confirmation modal (rendered outside the dropdown) */}
    {showClearConfirm && (
      <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
        <div className="bg-gray-800 rounded-lg p-6 max-w-sm w-full shadow-xl text-center">
          ...
          <p className="text-gray-400 mb-6">Are you sure you want to delete all notifications?</p>
          <div className="flex space-x-3 justify-center">
            <button onClick={() => setShowClearConfirm(false)} className="px-5 py-2 bg-gray-600 text-white rounded hover:bg-gray-500 transition-colors">Cancel</button>
            <button onClick={() => { clearAll(); setShowClearConfirm(false); }} className="px-5 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition-colors">Clear All</button>
          </div>
        </div>
      </div>
    )}
