...
const Whiteboard = ({ groupId }) => {
  ...
  const [activePens, setActivePens] = useState({}); // { socketId: { x, y, color, name, lastSeen } }
  const penTimeoutsRef = useRef({});
  ...
  useEffect(() => {
    ...
    if (socket) {
      ...
      socket.on("drawing_active", ({ x, y, color, userName, socketId }) => {
        // Clear any existing inactivity timeout for this pen
        if (penTimeoutsRef.current[socketId]) {
          clearTimeout(penTimeoutsRef.current[socketId]);
        }
        setActivePens((prev) => ({
          ...prev,
          [socketId]: { x, y, color, userName }
        }));
        // Auto-remove pen after 5 seconds of inactivity (handles disconnects without event)
        penTimeoutsRef.current[socketId] = setTimeout(() => {
          setActivePens((prev) => {
            const copy = { ...prev };
            delete copy[socketId];
            return copy;
          });
          delete penTimeoutsRef.current[socketId];
        }, 5000);
      });
    ...
    return () => {
      ...
      // Clear all pen inactivity timers on unmount
      Object.values(penTimeoutsRef.current).forEach(clearTimeout);
      penTimeoutsRef.current = {};
    };
  }, [socket]);
