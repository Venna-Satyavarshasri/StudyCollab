...
const GroupPage = () => {
  ...
  useEffect(() => {
    if (group) document.title = `${group.name} – Study Planner`;
  }, [group]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleGroupDeleted = (deletedGroupId) => {
      if (deletedGroupId === groupId) {
        setFlashMessage({ type: 'error', text: 'This group has been deleted by the admin.' });
    ...
    const handleMemberRemoved = ({ memberId, group: updatedGroup }) => {
      ...
      if (memberId === user._id) {
        setFlashMessage({ type: 'error', text: 'You have been removed from this group.' });
        setTimeout(() => navigate('/dashboard'), 2000);
      }
    };
  ...
  const handleDeleteGroup = async () => {
    try {
      ...
      setFlashMessage({ type: 'error', text: 'Failed to delete group. Please try again.' });
      setShowDeleteConfirm(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gray-900 text-white p-8">
        <div className="w-full max-w-6xl mx-auto animate-pulse">
          <div className="h-9 bg-gray-700 rounded w-1/3 mb-6"></div>
          <div className="flex space-x-2 mb-6">
            {[...Array(4)].map((_, i) => <div key={i} className="h-8 bg-gray-700 rounded w-24"></div>)}
          </div>
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-8">
      {/* Flash message toast */}
      {flashMessage && (
        <div className={`fixed top-4 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded shadow-lg text-white font-medium ${
          flashMessage.type === 'error' ? 'bg-red-600' : 'bg-green-600'
        }`}>
          {flashMessage.text}
        </div>
      )}
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
          <div className="bg-gray-800 rounded-lg p-6 max-w-sm w-full shadow-xl text-center">
            ...
            <p className="text-gray-400 mb-6">Are you sure you want to delete this group? All tasks and events will be permanently removed.</p>
            <div className="flex space-x-3 justify-center">
              <button onClick={() => setShowDeleteConfirm(false)} className="px-5 py-2 bg-gray-600 text-white rounded hover:bg-gray-500 transition-colors">Cancel</button>
              <button onClick={handleDeleteGroup} className="px-5 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition-colors">Delete</button>
            </div>
          </div>
        </div>
      )}
